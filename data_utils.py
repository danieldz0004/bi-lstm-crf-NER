...
class BatchManager(object):
    ...
    def sort_and_pad(self, data, batch_size):
        ...
        for i in range(num_batch):
            slice_data = sorted_data[int(i * batch_size): int((i + 1) * batch_size)]
            batch_data.append(self.pad_data(slice_data))  # (1044,4,20)
    ...
    for s in sentences:
        ...
        if train:
           ...
           # tags = [tag_to_id[w[-1]] for w in s]
            ##  add by devin 2018/12/27

            tags = []
            for w in s:
                label = w[-1]
                tag = tag_to_id.get(label)
                if tag is None:
                    sign = False
                    print('\n\n\n\n\n')
                    print("error = ", label)
